...
import { formatReceived, providerPayout } from "./OrderStore.jsx";
import { RequestProvider, useRequests } from "./RequestStore.jsx";
import { useOffers } from "./OfferStore.jsx";
import { BrowserChrome } from "./Inspectd.jsx";
...
/*
 * Dispatch turns a researched job into one shareable link. One link for the
 * job, not one per provider — you text the same URL to whoever you called, and
 * the first to claim it gets it.
 */
function DispatchSection({ job, onOpenTab }) {
  const { dispatch, findDispatch } = useOffers();
  ...
  return (
    <section>
      <h3 className="text-base font-semibold text-slate-900">Dispatch</h3>
      ...
      {!sent ? (
        ...
        <>
          ...
          <div className="mt-2 flex gap-2">
            <Btn size="sm" onClick={doCopy}>
              <Copy className="h-3.5 w-3.5" />
              {copied ? "Copied" : "Copy link"}
            </Btn>
            <Btn size="sm" onClick={() => onOpenTab(sent.tokenId)}>
              <ExternalLink className="h-3.5 w-3.5" />
              Open
            </Btn>
          </div>
          <p className="mt-2 text-xs text-slate-500">
            Dispatched {formatReceived(sent.token.dispatchedAt)}
            {sent.job.claimedByPhone
              ? " · Claimed by " + sent.job.claimedByPhone
              : sent.job.state !== "OPEN"
              ? " · " + sent.job.state.toLowerCase().replace(/_/g, " ")
              : " · not yet claimed"}
          </p>
        </>
      )}
...
function DrawerBody({ job, onClose, onOpenJob, onOpenTab }) {
  ...
  return (
    <>
      ...
      <div className="flex-1 space-y-6 overflow-y-auto px-6 py-5">
        ...
        <DispatchSection job={job} onOpenTab={onOpenTab} />
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <div className="flex items-center gap-1.5">
              <span className="truncate text-[11px] text-slate-500">{job.customer.email}</span>
              {duplicate && (
                <AlertTriangle
      ...
      <Drawer open={!!selected} onClose={() => setSelectedId(null)}>
        {selected && (
          <DrawerBody
            ...
            onOpenTab={setTabToken}
          />
        )}
      </Drawer>

      {/* Same simulated-tab treatment App.jsx gives the dealer order flow. */}
      {tabToken && (
        <div className="absolute inset-0 z-50">
          <BrowserChrome
            url={"inspectd.com/job/" + tabToken}
            onClose={() => setTabToken(null)}
          >
            <JobOffer token={tabToken} />
          </BrowserChrome>
        </div>
      )}
    </div>
